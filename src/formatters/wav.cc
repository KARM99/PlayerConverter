...
extern "C" AudioData AudioRead(const char* filename) {
    ...
    file.read(&ad.data[0], ad.data.size());
    return ad;
}
...
extern "C" void AudioWrite(const AudioData& ad, const char* filename) {
    WAV_HEADER header {
        ...
        ad.num_channels,
        ad.sample_rate,
        std::uint32_t{ad.sample_rate} * uint32_t{ad.num_channels} * uint32_t{(ad.bits_per_sample / 8)},
        ad.num_channels * (ad.bits_per_sample / 8),
        ad.bits_per_sample,
        {'d', 'a', 't', 'a'},
        ad.data.size()
    };
